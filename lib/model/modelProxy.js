...
ModelProxy.prototype.$packResultSet = function(resultSet) {
	...
	for (var dataKey in dataResult) {
		...
		if (Utils.checkFunction(dataValue) || Utils.checkAsyncFunction(dataValue)) {
			continue;
		}
...
ModelProxy.prototype._modelInit = function() {
	...
	if (Utils.checkFunction(func) || Utils.checkAsyncFunction(func)) {
		var proto = func.prototype;
		for (interface in proto) {
			if (Utils.checkFunction(proto[interface]) || Utils.checkAsyncFunction(proto[interface])) {
				(function(method) {
 ...
 *
 * @param method {String} invoke method name
 * @param args {Array}  invoke arguments
 * @return {Object} invoke result
 * @api private
 */
ModelProxy.prototype._modelInvoke = function(method, args) {
	var targetModel = this.model;
	if (Utils.checkFunction(targetModel[method]) || Utils.checkAsyncFunction(targetModel[method])) {
