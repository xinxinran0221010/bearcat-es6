...
DynamicAopProxy.prototype.dyInit = function() {
	...
	for (var i = 0; i < interfaces.length; i++) {
		(function(interfaceName) {
			if (checkFuncName(interfaceName)) {
				logger.error('init error proxy method interface %j the same as DynamicAopProxy, rename this name to another.', interfaceName)
				return;
			}

			self[interfaceName] = function() {
				var args = Array.prototype.slice.apply(arguments);
				return self.dyInvoke(interfaceName, args);
			}
		})(interfaces[i]);
	}
}
...
DynamicAopProxy.prototype.dyInvoke = function(method, args) {
	...
	var flag = false;
	if (!Utils.checkFunction(invokeCb) || !Utils.checkAsyncFunction(invokeCb)) {
...
DynamicAopProxy.prototype.doInvokeAdvisorsBefore = function(method, args, advisors, cb) {
	...
	var next = function(err) {
		...
		if (advisor.isRuntime()) {
			...
			return aspectBean[advise](_next);
		}
	}
